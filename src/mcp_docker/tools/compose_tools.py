...
class ComposeWriteFileTool(ComposeBaseTool):
    ...
    def description(self) -> str:
        """Tool description."""
        return (
            "Write a Docker Compose file to the compose_files directory with "
            "automatic validation and best practice checks.\n\n"
            "BEST PRACTICES - Follow these for reliable services:\n"
            "✅ DO:\n"
            "  • Use official stable images (nginx:alpine, postgres:15-alpine, "
            "redis:7-alpine)\n"
            ...
            "❌ AVOID:\n"
            "  • Complex inline code with 'python -c', 'node -e', 'ruby -e' - "
            "these are fragile!\n"
            ...
            "EXAMPLES:\n"
            "Good: image: nginx:alpine, ports: ['80:80'], "
            "volumes: ['./html:/usr/share/nginx/html']\n"
            "Bad:  command: python -c 'from flask import Flask; "
            "app=Flask(__name__); ...' # Will likely fail!\n\n"
            "Better approach for custom apps: Use a Dockerfile with "
            "COPY app.py /app/ and CMD ['python', '/app/app.py']\n\n"
            "The tool validates syntax but cannot detect runtime errors in "
            "application code. You'll receive warnings for anti-patterns and "
